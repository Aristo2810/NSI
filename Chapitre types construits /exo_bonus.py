...
def decryptage(num):
    ...
    3 <--> 7
    4 <--> 6
    :param str num: le numéro de téléphone à décrypter / crypter
    :return: Le num de tel décrypté / crypté selonla symétrie des nums sur un clavier
    :rtype: str
    """
    nouveau_num = ''
    table = [5, 8, 6, 4, 2, -5, -2, -4, -6, -8]
    for cara in num:
        nouveau_num += str(int(cara) + table[int(cara)])
    return nouveau_num


assert decryptage('0123456789') == '5987604321'
assert decryptage('') == ''
assert decryptage('0123481055') == '5987629500'
